...
import { Tile, IconBoardSettings } from './types';
...
export function needsMigration(settings: IconBoardSettings): boolean {
  return !settings.v2migrationDone && settings.rootTiles.length > 0;
}

/**
 * Converts v1 plugin-settings tiles into `.iboard` files.
 * Nested board tiles become separate `.iboard` files.
...
export async function migrateV1toV2(app: App, plugin: IconBoardPlugin): Promise<TFile> {
  ...
  new Notice(
    `Icon Board: Your tiles have been migrated to "${homeFile.path}". ` +
...
async function convertBoard(app: App, tiles: Tile[], boardName: string): Promise<TFile> {
  ...
  const board: IconBoardFile = { version: 2, layout: 'grid', cards };
