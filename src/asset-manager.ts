...
export async function relinkBoardData(app: App, board: IconBoardFile): Promise<number> {
  let fixed = 0;
...
// Scans every .iboard file in the vault, fixes broken paths, saves changed files.
// Returns the total number of links fixed across all boards.
export async function relinkAllBoards(app: App): Promise<number> {
  const boards = app.vault.getFiles().filter(f => f.extension === 'iboard');
  let total = 0;
  for (const bf of boards) {
    try {
